...
edu.sharif.selab.models.TelegramMessage

۲

افزودن کلاس

edu.sharif.selab.services.TelegramMessageService
...
کلاس Main برای افزودن قابلیت جدید (تلگرام) مستقیماً ویرایش شد و برای قابلیت‌های بعدی نیز باید تغییر کند.

Liskov Substitution Principle

👎 نقض

زیرکلاس‌های MessageService (مانند SmsMessageService) رفتار متدهای اینترفیس را به طور کامل پیاده‌سازی نمی‌کنند (بدنه‌های خالی دارند) که منجر به رفتار غیرمنتظره می‌شود.

Interface Segregation Principle

👎 نقض

اینترفیس MessageService یک "اینترفیس فربه" است و کلاس‌ها را مجبور به پیاده‌سازی متدهایی می‌کند که به آن‌ها نیازی ندارند.

Dependency Inversion Principle

👎 نقض

کلاس Main (ماژول سطح بالا) به طور مستقیم به کلاس‌های سرویس ملموس مانند new SmsMessageService() (ماژول‌های سطح پایین) وابسته است.
...
اینترفیس MessageService فربه و کلی است و باعث ایجاد متدهای خالی در کلاس‌های فرزند می‌شود.

شکستن اینترفیس فربه. <br> ایجاد یک اینترفیس Generic به نام MessageService<T extends Message> با یک متد واحد send(T message).

OCP & DIP

Main به کلاس‌های ملموس وابسته است و برای افزودن سرویس جدید باید کد آن تغییر کند.

استفاده از الگوی طراحی Factory. <br> ایجاد کلاس MessageServiceFactory برای کپسوله کردن منطق ساخت سرویس‌ها و حذف وابستگی مستقیم Main به کلاس‌های سرویس ملموس.
...
اگر اصول شی‌گرایی از ابتدا برقرار بود، یک مورد از تغییرات ثبت شده در جدول (مورد شماره ۳: تغییر در تابع main) حذف می‌شد.
در آن صورت، افزودن سرویس جدید تنها با ۳ تغییر در سطح کد انجام می‌شد:

ایجاد کلاس مدل TelegramMessage.java (تغییر ۱).

ایجاد کلاس سرویس TelegramMessageService.java (تغییر ۲).

افزودن یک خط کد برای ثبت سرویس جدید در کلاس MessageServiceFactory.java (تغییر ۳).

۲. با توجه به آنچه که انجام دادید، در دو خط توضیح دهید که رعایت اصول شی‌گرایی، چه مزایایی را برای پروژه شما فراهم می‌کند؟

رعایت اصول شی‌گرایی، قابلیت توسعه‌پذیری و نگهداری پروژه را به شدت افزایش می‌دهد، زیرا می‌توان بدون دستکاری کدهای موجود، قابلیت‌های جدیدی به آن اضافه کرد. همچنین با کاهش وابستگی‌ها و خواناتر شدن کد، درک و استفاده مجدد از آن در آینده ساده‌تر می‌شود.